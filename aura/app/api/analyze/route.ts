...
function vibe(scores: number[]): string {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const modeInstructions = mode === "roast"
      ? "Be brutally honest and slightly savage but still helpful. Don't hold back."
      : mode === "glam"
      ? "Judge this as if it's a red carpet event. Very high standards."
...
You analyze outfits and return ONLY valid JSON — no markdown, no extra text.`
...
Return ONLY valid JSON:
{
  ...
  "ai_tip": "<one specific, actionable style tip to improve the look — 1 sentence>",
  "ai_review": "<2-3 sentence overall review of the outfit — enthusiastic, specific, shareable>"
}`
    ...
    const vibeLabel = vibe([scores.style_score, scores.color_score, scores.originality_score, scores.impact_score])
