...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("cdm_lang") as Lang | null;
  ...
  const setLang = (l: Lang) => {
    setLangState(l);
    localStorage.setItem("cdm_lang", l);
  };
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className="absolute right-0 top-full mt-1 z-50 rounded-xl overflow-hidden py-1 min-w-[110px]"
            style={{
              background: "#161b27",
              border: "1px solid #1e2a3d",
