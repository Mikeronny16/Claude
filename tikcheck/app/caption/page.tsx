...
export default function CaptionPage() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#000" }}>
      <BackHeader title="✍️ Caption Enhancer" color="#00F2EA" />
      <div style={{ padding: "20px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
          {PLATFORMS.map(p => (
            <button key={p} onClick={() => setPlatform(p)} style={{
              padding: "7px 14px", borderRadius: 20, border: `1px solid ${platform === p ? "#00F2EA" : "rgba(255,255,255,0.08)"}`,
              background: platform === p ? "rgba(0,242,234,0.12)" : "transparent",
              color: platform === p ? "#00F2EA" : "#555", fontSize: 12, fontWeight: 600,
        ...
        <motion.button whileTap={{ scale: 0.96 }} onClick={enhance} disabled={loading || !caption.trim()}
          style={{
            ...
            background: loading || !caption.trim() ? "#1a1a1a" : "linear-gradient(135deg, #00F2EA, #00c4be)",
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ marginTop: 24 }}>
              <div style={{ background: "rgba(0,242,234,0.05)", border: "1px solid rgba(0,242,234,0.2)", borderRadius: 16, padding: "18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#00F2EA", letterSpacing: "1.5px" }}>ENHANCED CAPTION</span>
                  <button onClick={copy} style={{
                    ...
                    borderRadius: 8, padding: "5px 12px", color: copied ? "#00F2EA" : "#555",
                ...
                {result.cta && (
                  <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(255,255,255,0.04)", borderRadius: 10 }}>
                    ...
                    <span style={{ fontSize: 13, color: "#00F2EA" }}>{result.cta}</span>
                  </div>
                )}
              </div>
            </motion.div>
