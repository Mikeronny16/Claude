...
function IconHome({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? "#FF0050" : "#444"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
function IconTags({ active }: { active: boolean }) {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? "#FF0050" : "#444"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      <circle cx="7" cy="7" r="1.2" fill={active ? "#FF0050" : "#444"} stroke="none"/>
    </svg>
  );
}
        ...
        return (
          <Link key={href} href={href} style={{ textDecoration: "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minWidth: 52, position: "relative" }}>
              ...
              {active && (
                <div style={{ position: "absolute", bottom: -12, width: 18, height: 2, borderRadius: 2, background: "#FF0050" }} />
              )}
            </div>
          </Link>
        );
